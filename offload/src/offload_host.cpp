...
extern "C"
void __kmpc_proxy_task_completed_ooo(const void *thunk_ptr);
...
extern "C" {
    ...
    {
        OFFLOAD_TRACE(2, "Call OpenMP function _kmpc_proxy_task_completed_ooo(%p)\n", info);
        __kmpc_proxy_task_completed_ooo(info);
    }
}
...
)
{
   ...
   if (ompt_enabled()) {
    ...
    if (ompt_enabled() && ompt_get_target_data_map_begin_callback()) {
        ompt_target_map_entry_t *items = (ompt_target_map_entry_t*)
            malloc(sizeof(ompt_target_map_entry_t) * m_vars_total);
        
        ompt_target_map_entry_t *item = items;
        nitems = 0;
        for (int i = 0; i < m_vars_total; i++) {
            if (m_vars[i].direction.in) {
                switch (m_vars[i].type.dst) {
                    ...
                    case c_cean_var_ptr:
                        ...
                        if (signal == 0) {
                            item->mapping_flags |= ompt_target_map_flag_sync;
                        }
...
#ifdef OMPT_SUPPORT
    if (ompt_enabled() && nitems != 0 && ompt_get_target_data_map_end_callback()) {
        ompt_get_target_data_map_end_callback()(device_id, map_id);
    }
#endif
...
bool OffloadDescriptor::compute(void *info)
{
    ...
    if (ompt_enabled()) {
        ...
        if (ompt_get_new_target_task_callback(ompt_event_target_invoke_begin)
            && m_need_runuserfunction) {
            device_id = m_device.get_logical_index();
            task_id = ompt_get_task_id(0);
            ompt_get_new_target_task_callback(ompt_event_target_invoke_begin)(task_id,
                                         target_task_id,
                                         device_id,
    ...
    if (ompt_enabled()) {
        ...
        if (ompt_get_target_callback(ompt_event_target_invoke_end) && m_need_runuserfunction) {
            device_id = m_device.get_logical_index();
            task_id = ompt_get_task_id(0);
            ompt_get_target_callback(ompt_event_target_invoke_end)(task_id, target_task_id);
        }
...
extern "C" bool __offload_register_image(const void *target_image)
{
    ...
    switch (hdr->e_type) {
        ...
        {
            char * fullname = NULL;
            // Check to see if the library is a valid fat library
            if (offload_verify_library(origin, &fullname)) {
                if (fullname == NULL) {
                    fullname = origin;
                }
